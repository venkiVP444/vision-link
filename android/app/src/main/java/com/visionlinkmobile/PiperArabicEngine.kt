...
/**
 * Offline Piper Neural Text-to-Speech Engine for Arabic (Emirati Female)
 *
 * Runs the verified vadimbelsky/arabic-emirati-female-piper VITS ONNX model.
 * Executes 100% on-device on CPU with zero cloud/network dependency once installed.
 *
 * Primary Voice: Emirati Female
 * Sample Rate: 22,050 Hz
 */
class PiperArabicEngine private constructor(private val context: Context) {

    companion object {
        private const val TAG = "PiperArabicEngine"
        private const val PACK_DIR = "voice_packs/arabic"
        private const val MODEL_FILENAME = "model.onnx"
        private const val TOKENS_ASSET = "offline_tts/arabic_tokens.json"
        const val SAMPLE_RATE = 22050

        @Volatile
        private var INSTANCE: PiperArabicEngine? = null

        fun getInstance(context: Context): PiperArabicEngine {
            return INSTANCE ?: synchronized(this) {
                INSTANCE ?: PiperArabicEngine(context.applicationContext).also { INSTANCE = it }
            }
        }
    }
    ...
    fun isModelInstalledAndVerified(): Boolean {
        ...
        return file.exists() && ModelVerifier.verifyChecksum(file, ModelVerifier.SHA256_ARABIC)
    }
    ...
    fun initialize(): Boolean {
        ...
        try {
            ...
            Log.d(TAG, "Initializing Piper Arabic Engine...")

            val modelFile = getModelFile()
            if (!modelFile.exists()) {
                Log.w(TAG, "Arabic model file does not exist at ${modelFile.absolutePath}")
                return false
            }

            if (!ModelVerifier.verifyChecksum(modelFile, ModelVerifier.SHA256_ARABIC)) {
                Log.e(TAG, "Arabic model failed SHA-256 verification!")
                return false
            }
            ...
            val env = OrtEnvironment.getEnvironment("PiperArabic")
            ortEnv = env
            ...
            Log.i(TAG, "ONNX_INIT: SUCCESS. Piper Arabic Engine initialized in ${elapsed}ms. Model: ${modelFile.absolutePath}")
            return true
        } catch (e: Exception) {
            Log.e(TAG, "Failed to initialize Piper Arabic Engine: ${e.message}", e)
    ...
    private fun loadTokenMap() {
        try {
            ...
            Log.d(TAG, "Loaded ${phraseTokenMap.size} precomputed token sequences for Arabic")
        } catch (e: Exception) {
            Log.w(TAG, "Could not load arabic_tokens.json: ${e.message}")
        }
    }
    ...
    fun speak(text: String, utteranceId: String, callback: Callback?) {
        executor.execute {
            if (!isInitialized.get() || ortSession == null) {
                val ok = initialize()
                if (!ok) {
                    callback?.onError(utteranceId, "VOICE_PACK", "MODEL_NOT_READY", "Arabic voice pack is not installed or failed SHA-256 verification")
                    return@execute
                }
            }
            ...
            try {
                ...
                Log.d(TAG, "INFERENCE: SUCCESS. Arabic synthesis completed in ${elapsedInference}ms for utterance: $utteranceId")
                ...
                Log.e(TAG, "Arabic synthesis or playback failed: ${e.message}", e)
                isSpeaking.set(false)
                callback?.onError(utteranceId, "SYNTHESIS", "SYNTHESIS_ERROR", e.message ?: "Arabic synthesis failed")
            }
        }
    }
